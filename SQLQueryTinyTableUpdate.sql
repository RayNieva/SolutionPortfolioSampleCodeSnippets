...
IF EXISTS (SELECT 1 
    ...
    AND TABLE_NAME='temp_Order_Record_tiny2')
		...
		SELECT     Order_Record.Create_time, Order_Record.Update_time, Order_Record.URN_text, Order_Record.Status, Order_Record.Order_ID, 
							  Order_Row.Order_Row_ID, Order_Record.First_insert_date, Order_Record.Last_insert_date, Order_Record.Customer_ID, Order_Record.Sales_Area_ID,
							   Order_Record.Price, Order_Record.Payment_Type_ID, Order_Record.URN_number, Order_Record.Creator_ID
		INTO            temp_Order_Record_tiny2
		FROM         Order_Record INNER JOIN
							  Order_Row ON Order_Record.Order_ID = Order_Row.Order_ID
		WHERE     (Order_Record.Create_time >= DATEADD(n, - 60, GETDATE())) AND (Order_Record.Create_time <= GETDATE()) OR
							  (Order_Record.Update_time >= DATEADD(n, - 60, GETDATE())) AND (Order_Record.Update_time <= GETDATE())
		...
		(
		SELECT     Insert_Date, Order_Row_ID, Insert_net_price, Create_time, Update_time, URN_text, Sales_Area_ID, Order_ID,
		...
		Manager_ID, Team, TeamS, Name, Cust_URN_text, Cust_URN_number, Customer_Account_ID, Usual_Canvasser_ID, Account_number, 
		Product_ID, Planned_Rectangle_ID, Sequential_number, Order_Text_ID, Section_number, testSeqNumNull, testSectNumNull, Col, 
		CHAR(64 + testSectNumNull) AS page1, CAST(testSeqNumNull AS char(2)) AS page2, CustNameAcct, PosX, AtSize, FullName, 
		Telephone_std, Telephone_number, Telephone_prefix, Telephone_extension
		FROM          

		(
		SELECT     oRec9.Insert_Date, oRec9.Order_Row_ID, oRec9.Insert_net_price, oRec9.Create_time, oRec9.Update_time, oRec9.URN_text, 
		...
		oRec9.Edzone_ID,oRec9.Pgroup_ID,oRec9.Class_ID,oRec9.Style_ID,oRec9.Size_ID,oRec9.Colour_ID,oRec9.Position_ID,--oRec9.Order_Text_ID, 

		oRec9.Stop_date, oRec9.Stop_number, oRec9.HJ_pages, oRec9.HJ_columns, 
		oRec9.HJ_width, oRec9.HJ_depth, oRec9.HJ_natural_depth, oRec9.HJ_lines, oRec9.HJ_depth / 2.54 AS Depth, 
		...
		oRec9.Setting, oRec9.Size, oRec9.SizeS, oRec9.Color, oRec9.ColorS, oRec9.Position, oRec9.Pos, oRec9.Text_sample, 
		...
		oRec9.RepCode, oRec9.Forenames, oRec9.Surname, oRec9.Team_ID, oRec9.Manager_ID, oRec9.Team, oRec9.TeamS, 
		oRec9.Name, oRec9.Cust_URN_text, oRec9.Cust_URN_number, oRec9.Customer_Account_ID, oRec9.Usual_Canvasser_ID, 
		oRec9.Account_number, oRec9.Product_ID, oRec9.Planned_Rectangle_ID, oRec9.Sequential_number, oRec9.Order_Text_ID, 
		oRec9.Section_number, ISNULL(oRec9.Sequential_number, ' ') AS testSeqNumNull, ISNULL(oRec9.Section_number, ' ') 
		...
		oRec9.HJ_columns * oRec9.HJ_depth * (CASE WHEN HJ_pages > 0 THEN HJ_pages ELSE 1 END) AS AtSize, 
		oRec9.Forenames + ' ' + oRec9.Surname AS FullName, Operator_Telephone.Telephone_std, 
		Operator_Telephone.Telephone_number, Operator_Telephone.Telephone_prefix,Operator_Telephone.Telephone_extension
		...
		Order_Row.Edzone_ID,Order_Row.Pgroup_ID,Order_Row.Class_ID,Order_Row.Style_ID,Order_Row.Size_ID,Order_Row.Colour_ID,Order_Row.Position_ID,--Order_Row.Order_Text_ID,
		...
		FROM          temp_Planned_Advert RIGHT OUTER JOIN
		  Planned_Rectangle ON temp_Planned_Advert.Sequential_number = Planned_Rectangle.Sequential_number AND 
		  ...
		  temp_Order_Record_tiny2 ON Order_Insert.Order_Row_ID = temp_Order_Record_tiny2.Order_Row_ID INNER JOIN
		  ...
		  Customer_Account ON Customer.Customer_Account_ID = Customer_Account.Customer_Account_ID LEFT OUTER JOIN
		  Planned_Advert ON Order_Insert.Order_Row_ID = Planned_Advert.Order_Row_ID AND 
		  Order_Insert.Insert_Date = Planned_Advert.Insert_Date ON Title_Card.Title_ID = Order_Row.Title_ID LEFT OUTER JOIN
		...
		WHERE      (Order_Insert.Insert_Date >= DATEADD(dd, - 36, GETDATE())) AND (Order_Insert.Insert_Date <= DATEADD(dd, 30, 
		  GETDATE())) AND (temp_Order_Record_tiny2.Status = 'RUN') --AND (Style_Card.Style_type_setting = 'PRD')
		...
		--Sales_Area.Long_name AS SalesArea, Sales_Area.Short_name AS SalesAreaS, Sales_Area.Description AS SalesDescription, @From_Date, @To_Date
